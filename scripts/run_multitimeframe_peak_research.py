"""Run nested monthly/weekly/daily peak indicator research."""
...
def main() -> int:
    ...
    with sqlite3.connect(args.database.resolve().as_uri() + "?mode=ro", uri=True) as connection:
        ...
        sectors = {str(row[0]): str(row[1] or "未分類") for row in connection.execute("SELECT code,sector_33_name FROM master_stock")}
    config = yaml.safe_load((ROOT / "config/indicators.yaml").read_text(encoding="utf-8"))
    result = analyze_nested_peaks(frames, sectors, config)
