...
begin
  ...
  insert into public.short_pattern_results(
    run_id, position, code, company_name, pattern_id, pattern_label, direction, pattern_summary,
  ...
  select p_run_id, r.position, r.code, coalesce(r.company_name,''), r.pattern_id, r.pattern_label,
    r.direction, coalesce(r.pattern_summary,''), r.signal_date, r.signal_close, r.target_percent,
  ...
  from jsonb_to_recordset(p_results) as r(
    position integer, code text, company_name text, pattern_id text, pattern_label text, direction text,
