-- Add read-only display fields for two distinct historical entry timings.
-- Existing probabilities remain the "advance / next session open" definition.
-- Confirmation statistics stay inside horizon_statistics JSON to preserve rows.
begin;

alter table public.short_pattern_results
  add column if not exists confirmation_trigger_price double precision,
  add column if not exists confirmation_window_sessions integer;
...
begin
  ...
  insert into public.short_pattern_results(
    ...
    resistance_price, support_price, holding_days, target_probability_percent, trade_count, average_return_percent,
    median_return_percent, max_adverse_percent, out_of_sample_trade_count,
    out_of_sample_target_probability_percent, horizon_statistics, morning_price, morning_price_at,
    morning_target_price, confirmation_status
  )
  select p_run_id, r.position, r.code, coalesce(r.company_name,''), r.pattern_id, r.pattern_label,
    ...
    r.target_percent, r.target_price, r.confirmation_trigger_price, r.confirmation_window_sessions,
    r.resistance_price, r.support_price, r.holding_days, r.target_probability_percent, r.trade_count,
    r.average_return_percent, r.median_return_percent, r.max_adverse_percent, r.out_of_sample_trade_count,
    r.out_of_sample_target_probability_percent, coalesce(r.horizon_statistics, '{}'::jsonb), r.morning_price,
    r.morning_price_at, r.morning_target_price, coalesce(r.confirmation_status,'')
  from jsonb_to_recordset(p_results) as r(
    ...
    resistance_price double precision, support_price double precision, holding_days integer,
    target_probability_percent double precision, trade_count integer, average_return_percent double precision,
    median_return_percent double precision, max_adverse_percent double precision, out_of_sample_trade_count integer,
    out_of_sample_target_probability_percent double precision, horizon_statistics jsonb, morning_price double precision,
    morning_price_at timestamptz, morning_target_price double precision, confirmation_status text
  );
end $$;
